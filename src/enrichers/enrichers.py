...
class LLMEnricher:
    ...
    def _build_system_prompt(self) -> str:
        return (
            ...
            "Входной чанк — фрагмент статистического сборника, преимущественно таблицы "
            "(заголовки таблиц, строки, столбцы, единицы измерения, возможное отсутствие связного текста).\n\n"

            "По входному чанку верни ТОЛЬКО один валидный JSON-объект со следующими полями:\n"
            "- chunk_id: строка\n"
            "- context: строка, 1–2 предложения, кратко описывающие, что измеряется, по каким территориям и за какой период; без числовых значений\n"
            "- geo: список территорий, упомянутых в таблице или таблицах\n"
            "- metrics: список названий статистических показателей (без значений и единиц измерения)\n"
            "- years: список лет, явно упомянутых в чанке\n"
            "- time_granularity: 'year', 'quarter', 'month' или null\n"
            "- oked: строка или null\n\n"

            "Правила:\n"
            "1. Используй ТОЛЬКО информацию, явно присутствующую в чанке.\n"
            "2. НИЧЕГО не додумывай.\n"
            "3. Если поле невозможно определить — верни пустой список [] или null.\n"
            "4. Числовые значения показателей НИКОГДА не включай.\n"
            "5. В geo не включай агрегаты типа «Всего», «Итого», «В целом по стране».\n"
            "6. Если years пустой список — time_granularity = null.\n"
            "7. Ответ должен содержать ТОЛЬКО JSON, без комментариев до или после.\n\n"

            "Пример:\n"
            "{\n"
            '  "chunk_id": "123",\n'
            '  "context": "Численность населения по регионам за 2019–2021 годы",\n'
            '  "geo": ["Минская область"],\n'
            '  "metrics": ["Численность населения"],\n'
    ...
    def _build_prompt(self, pdf_name: str, chunk: Chunk) -> str:
        ...
        return (
            ...
            f"Содержимое чанка:\n{text_snip}\n\n"
            "Сформируй JSON-объект согласно инструкции system prompt"
        )
